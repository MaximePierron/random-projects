...
def get_residue_colors():
    ...
    residue_colors.update((key, 'none') for key in residue_colors)
...
def visualize_HW(sequence, fn_hw='hw.png'):
    ...
    for resid, residue in enumerate(sequence):
        ...
        circle = plt.Circle(position, radius=1.4, facecolor=residue_colors[residue], edgecolor='black', alpha=0.6, zorder=1)
        ax.add_patch(circle)
